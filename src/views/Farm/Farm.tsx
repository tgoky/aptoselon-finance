...
const Farm: React.FC = () => {
  ...
  const RedeemButton = useMemo(() => {
    if (status !== "connected") {
      return <Button disabled text="Harvest &amp; Unstake AEON/APT" variant="secondary" />;
    }
    if (!isRedeeming) {
      return <Button onClick={onRedeemYAMETH} text="Harvest &amp; Unstake AEON/APT" variant="secondary" />;
    }
  ...
  return (
    <Page>
      <PageHeader icon={`${farmer}`} subtitle="Stake AEON/APT Sushiswap LP tokens and grow APTOSELONs" title="Farm" />
      <Container>
        ...
        <Split>
          ...
          <Button
            full
            text="Get APTELON/APT LP tokens"
